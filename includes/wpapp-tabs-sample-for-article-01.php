...
class WPCD_WordPress_TABS_APP_SAMPLE extends WPCD_WORDPRESS_TABS {
	...
	private function sample_action_a( $id, $action ) {
		...
		$command = "cd /var/www/$domain/html && pwd && sudo -u $domain  wp media regenerate --yes";
		...
		if ( ! ( strpos( $result, 'Success: Regenerated' ) ) && ! ( strpos( $result, 'Warning: No images found.' ) ) ) {
		...
		$success_msg = __( 'Command was a success - Images Regenerated!', 'wpcd' );
		$result      = array(
			'msg'     => $success_msg,
			'refresh' => 'yes',
		);
